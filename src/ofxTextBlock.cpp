...
void ofxTextBlock::wrapTextArea(float rWidth, float rHeight){
	...
	if (words.size() > 0) {
		...
		for (int iteration=1; iteration <= maxIterations; iteration++){
			...
			if ((tmpScale * getHeight()) < rHeight) 		scales[iteration] = tmpScale;
			else											scales[iteration] = -1;
		}
		...
		for (int i=1; i <= maxIterations; i++) {
			...
			if (scales[i] > scales[maxIndex]) 	maxIndex = i;
		}
		...
		if (bScaleAvailable) 		scale = scales[maxIndex];
		else						scale = (float)rHeight / (float)getHeight();
...
float ofxTextBlock::getHeight(){
	if (words.size() > 0) 		return defaultFont.getLineHeight() * scale * lines.size();
	else 						return 0;
}
